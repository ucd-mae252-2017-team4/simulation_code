    ...
    for mission_idx in range(1,n_mission+1): #4
        ...
        for crew_idx in range(1,n_crew+1): #5
            ...
            for func_name in funcs:
                nodelist = astar.astar(mission_idx,crew_idx,func_name=='proxemic')
                path = viz.path_to_trajectory(nodelist) 

                viz.draw_path(path,mission,crew)
                plt.savefig('astar_path__mission%d_crew%d_%s.png' % (mission_idx,crew_idx,func_name))
                # plt.close()
                ...
                path_df[['vn','an']].plot()
                plt.tight_layout()
                plt.savefig('astar_velocity_accel__mission%d_crew%d_%s.png' % (mission_idx,crew_idx,func_name))
                plt.close()
                ...
                plt.tight_layout()
                plt.savefig('astar_crew_dist__mission%d_crew%d_%s.png' % (mission_idx,crew_idx,func_name))
                # plt.close()
...
def compare_damp_amp():
    ...
    for k_exp in range(-4,1):
        for b_val in [10, 5, 1, 0]:
            ...
            plt.savefig('path__amp%d_damp%d.png' % (k_exp,b_val))
            plt.close()
            ...
            path_df[['vn','fn']].plot()
            plt.tight_layout()
            plt.savefig('velocity_force__amp%d_damp%d.png' % (k_exp,b_val))
            plt.close()
            ...
            path_df[path_df.columns[path_df.columns.str.startswith('distance from crew ')]].plot()
            plt.tight_layout()
            plt.savefig('crew_distance__amp%d_damp%d.png' % (k_exp,b_val))
            plt.close()
            ...
            # plt.ylim((0,parameters.module_size[1]))
            # plt.tight_layout()
            # plt.savefig('quiver__amp%d_damp%d.png' % (k_exp,b_val))
            # plt.close()
